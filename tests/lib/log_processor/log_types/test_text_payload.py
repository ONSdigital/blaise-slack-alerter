...
from lib.log_processor.log_types.text_payload import attempt_create
...
def test_attempt_create_returns_processed_text_payload(log_entry):
    ...
    assert instance.message == "Example text payload"
    assert instance.data == {}
    assert instance.platform == "unknown-resource"
    assert instance.application is None


def test_attempt_create_returns_none_payload_type_is_not_text(log_entry):
    log_entry = dataclasses.replace(log_entry, payload_type=PayloadType.JSON)
    instance = attempt_create(log_entry)
    assert instance is None


def test_attempt_create_returns_none_payload_is_not_text(log_entry):
    log_entry = dataclasses.replace(log_entry, payload={})
    instance = attempt_create(log_entry)
    assert instance is None
