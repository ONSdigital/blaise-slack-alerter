...
@pytest.fixture()
def processed_log_entry_execute_sql_info() -> ProcessedLogEntry:
    return ProcessedLogEntry(
        message="cloudsql.instances.executeSql",
        data=dict(methodName="cloudsql.instances.executeSql"),
        severity="INFO",
...
def test_log_is_skipped_when_its_from_cloud_function_when_execute_sql_info(
    processed_log_entry_execute_sql_info: ProcessedLogEntry,
):
    log_is_skipped = execute_sql_filter(processed_log_entry_execute_sql_info)
    assert log_is_skipped is True
...
def test_log_message_is_not_a_string_when_execute_sql_info(
    processed_log_entry_execute_sql_info: ProcessedLogEntry,
):
    processed_log_entry_execute_sql_info = dataclasses.replace(
        processed_log_entry_execute_sql_info, message=1234
    )
    log_is_skipped = execute_sql_filter(processed_log_entry_execute_sql_info)
...
def test_log_message_is_not_skipped_when_it_does_not_contain_execute_sql_info(
    processed_log_entry_execute_sql_info: ProcessedLogEntry,
):
    processed_log_entry_execute_sql_info = dataclasses.replace(
        processed_log_entry_execute_sql_info,
        data=dict(methodName="dummy"),
    )
    log_is_skipped = execute_sql_filter(processed_log_entry_execute_sql_info)
...
def test_log_message_is_not_skipped_when_it_contains_severity_critical(
    processed_log_entry_execute_sql_error: ProcessedLogEntry,
):
    processed_log_entry_execute_sql_error = dataclasses.replace(
        processed_log_entry_execute_sql_error, severity="CRITICAL"
    )
    log_is_skipped = execute_sql_filter(processed_log_entry_execute_sql_error)
...
def test_log_message_is_not_skipped_when_it_contains_severity_debug(
    processed_log_entry_execute_sql_info: ProcessedLogEntry,
):
    processed_log_entry_execute_sql_info = dataclasses.replace(
        processed_log_entry_execute_sql_info, severity="DEBUG"
    )
    log_is_skipped = execute_sql_filter(processed_log_entry_execute_sql_info)
